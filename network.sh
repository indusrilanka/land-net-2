...
function createOrgs() {
  ...
  #   set -x
  #   cryptogen generate --config=./organizations/cryptogen/crypto-config-land-registry.yaml --output="organizations"
  #   res=$?
  #   { set +x; } 2>/dev/null
  #   if [ $res -ne 0 ]; then
  ...
  #   set -x
  #   cryptogen generate --config=./organizations/cryptogen/crypto-config-registrar-general.yaml --output="organizations"
  #   res=$?
  #   { set +x; } 2>/dev/null
  #   if [ $res -ne 0 ]; then
  #     fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/land-registry/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating Land Registry Identities"

    createLandRegistry

    infoln "Creating Registrar General Identities"

    createRegistrarGeneral

    infoln "Creating Notary Identities"
  ...
  infoln "Generating CCP files for LandRegistry , RegistrarGeneral , Notary and Other organizations"
...
# call the script to create the channel, join the peers of land-registry and org2,
...
function networkDown() {
  ...
  # stop org3 containers also in addition to land-registry and org2, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/land-registry/msp organizations/fabric-ca/land-registry/tls-cert.pem organizations/fabric-ca/land-registry/ca-cert.pem organizations/fabric-ca/land-registry/IssuerPublicKey organizations/fabric-ca/land-registry/IssuerRevocationPublicKey organizations/fabric-ca/land-registry/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/registrar-general/msp organizations/fabric-ca/registrar-general/tls-cert.pem organizations/fabric-ca/registrar-general/ca-cert.pem organizations/fabric-ca/registrar-general/IssuerPublicKey organizations/fabric-ca/registrar-general/IssuerRevocationPublicKey organizations/fabric-ca/registrar-general/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/notary/msp organizations/fabric-ca/notary/tls-cert.pem organizations/fabric-ca/notary/ca-cert.pem organizations/fabric-ca/notary/IssuerPublicKey organizations/fabric-ca/notary/IssuerRevocationPublicKey organizations/fabric-ca/notary/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/other/msp organizations/fabric-ca/other/tls-cert.pem organizations/fabric-ca/other/ca-cert.pem organizations/fabric-ca/other/IssuerPublicKey organizations/fabric-ca/other/IssuerRevocationPublicKey organizations/fabric-ca/other/fabric-ca-server.db'
...
COMPOSE_FILE_BASE=compose-land-net.yaml
